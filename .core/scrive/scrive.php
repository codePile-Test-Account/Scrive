...
class scrive
{
    ...
    public function __construct()
    {
        ...
        // Load core modules
        $this->load_modules();
        $this->run_hooks('modules_loaded');
        
        // Load the settings
        $settings = $this->get_config();
        $this->run_hooks('config_loaded', array(
            &$settings
        ));
        ...
        $this->run_hooks('request_url', array(
            &$url
        ));
        ...
        $this->run_hooks('before_load_content', array(
            &$file
        ));
        if (file_exists($file)) {
            $content = file_get_contents($file);
        } else {
            $this->run_hooks('before_404_load_content', array(
                &$file
            ));
            $content = file_get_contents(CONTENT_DIR . '404' . CONTENT_EXT);
            header($_SERVER['SERVER_PROTOCOL'] . ' 404 Not Found');
            $this->run_hooks('after_404_load_content', array(
                &$file,
                &$content
            ));
        }
        $this->run_hooks('after_load_content', array(
            &$file,
            &$content
        ));
        
        $meta = $this->read_file_meta($content);
        $this->run_hooks('file_meta', array(
            &$meta
        ));
        
        $this->run_hooks('before_parse_content', array(
            &$content
        ));
        $content = $this->parse_content($content);
        $this->run_hooks('after_parse_content', array(
            &$content
        ));
        $this->run_hooks('content_parsed', array(
            &$content
        )); // Depreciated @ v0.8
        ...
        $this->run_hooks('get_pages', array(
            &$pages,
            &$current_page,
            &$prev_page,
            &$next_page
        ...
        $twig_vars = array(
            ...
            'theme_css'     => $settings['base_url'] . '/' . basename(THEMES_DIR) . '/' . $settings['theme'] . '/assets/css',
            'theme_js'      => $settings['base_url'] . '/' . basename(THEMES_DIR) . '/' . $settings['theme'] . '/assets/js',
            'theme_img'     => $settings['base_url'] . '/' . basename(THEMES_DIR) . '/' . $settings['theme'] . '/assets/img',
            'tpl_parts'     => $settings['base_url'] . '/' . basename(THEMES_DIR) . '/' . $settings['theme'] . '/tpl-parts',
            'core_css'      => $settings['base_url'] . '/' . '.core/scrive/modules/css-helpers',
            'core_js'       => $settings['base_url'] . '/' . '.core/scrive/modules/js-helpers',
            'core_tpl'      => $settings['base_url'] . '/' . '.core/scrive/modules/tpl-helpers',
        ...
        $view = (isset($meta['view']) && $meta['view']) ? $meta['view'] : 'base';
        $this->run_hooks('before_render', array(
            &$twig_vars,
            &$twig,
            &$view
        ));
        $output = $twig->render($view . '.html', $twig_vars);
        $this->run_hooks('after_render', array(
            &$output
        ));
        echo $output;
    }
    ...
    /**
     * Parses the file meta from the .md file header
     *
     * @param string $content the raw txt content
    ...
    protected function read_file_meta($content)
    {
        ...
        $headers = array(
            ...
            'description'  => 'Description', //Brief discription
            'date'         => 'Date', //Post date
            'category'     => 'Category', //Page category
            'slug'         => 'Slug', //Page slug -- keep lower case
            'author'       => 'Author', //The posting author
            'robots'       => 'Robots', //To allow page indexing
            'view'         => 'view' //Forchoosing a custom template
        );
        
        // Add support for custom headers by hooking into the headers array
        $this->run_hooks('before_read_file_meta', array(
            &$headers
        ));
        ...
        if (isset($headers['slug']))
            $headers['slug'] = strtolower(preg_replace('/-+/', '-', preg_replace('/[^\wáéíóú]/', '-', $headers['title'])));
    ...
    protected function get_config()
    {
        ...
        @include_once(THEMES_DIR . $config['theme'] . '/theme-config.php');
        
        $defaults = array(
            'site_title'  => 'scrive.io',
    ...
    protected function get_pages($base_url, $order_by = 'alpha', $order = 'asc', $excerpt_length = 50)
    {
        ...
        foreach ($pages as $key => $page) {
            ...
            $this->run_hooks('get_page_data', array(
                &$data,
                $page_meta
            ));
