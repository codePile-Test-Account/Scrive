 ...
 * Create a specific Twig variable to include a map in the template
 * @link http://docs.scrive.io/about/leaflet
 *
 * @pakage scrive
 * @subpakage .core.modules/leaflet
 * 
...
class Leaflet {
	...
	{
		$this->lmap_styles = $settings['base_url'] .'/'. basename(CORE_DIR) .'/'. basename(MODULE_DIR) .'/leaflet/leaflet.css';
		...
		if (isset($settings['leaflet']['local']))
		{
			if ($settings['leaflet']['local'] === true) {
				$this->leaflet_local_base = $settings['base_url'] .'/'. basename(CORE_DIR) .'/'. basename(MODULE_DIR) . '/leaflet/Leaflet/dist/';
			}
		}
		if (isset($settings['leaflet']['providers']))
		{
			...
			{
				$this->providers_script = $settings['base_url'] .'/'. basename(CORE_DIR) .'/'. basename(MODULE_DIR) .'/leaflet/leaflet-providers/leaflet-providers.js';
			}
		}
	...
	public function build_leaflet_head()
	{
		$plhead;
	...
	public function build_leaflet_map()
	{
		$idmap;
		if ($this->is_map === true)
		{
	...
	public function after_render(&$output)
	{
		if ($this->is_map === true) {
			...
			$output = str_replace('</head>', PHP_EOL.$this->build_leaflet_head().'</head>', $output);
			// Adding the map script before the end of the <body>
			$output = str_replace('</body>', PHP_EOL.$this->build_leaflet_map().'</body>', $output);
		}
		elseif (!$this->is_map === true && isset($this->lmap) && $this->lmap === 'article')
		{
			// Adding stylesheets and scripts for Leaflet inside <head>
